{
  ...
  RooConstVar alpha_gobs("alpha_gobs","alpha_gobs",1);RooConstVar alpha_sigma("alpha_sigma","alpha_sigma",0.1);
...
//first do without systematic
  ...
  cout << endl;
  cout << " red gradient = " << (c2.GetY()[c2.GetN()-6] - c2.GetY()[c2.GetN()-16])/(c2.GetX()[c2.GetN()-6] - c2.GetX()[c2.GetN()-16]) << std::endl;
  cout <<  " blue gradient = " << (c.GetY()[c.GetN()-6] - c.GetY()[c.GetN()-16])/(c.GetX()[c.GetN()-6] - c.GetX()[c.GetN()-16]) << std::endl;
  ...
  If underfit bin1, then N1 > n1+s1*mu, leading to gradient < s2
